        ...
        {UIUX.map((img, i) => (
          <div key={i}>
            <div onClick={() => {
              ...
              className="cursor-pointer focus:outline-none rounded-2xl outline outline-2 outline-neutral-300 overflow-hidden"
            >
              <Image
                src={img.image}
        ...
        ))}
        {
          ...
          <div className='fixed inset-0 flex items-center justify-center w-full h-full'>
            <div onClick={() => setShowImage(false)} className="fixed inset-0 bg-indigo-200/70 z-40 p-4">
              <HiOutlineX onClick={() => setShowImage(false)} className="cursor-pointer absolute top-2 left-2 text-5xl text-indigo-500" />
            </div>
